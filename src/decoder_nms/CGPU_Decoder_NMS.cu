...
#include "CGPU_Decoder_NMS.h"
#include "../transpose/GPU_Transpose.h"

//extern __global__ void LDPC_Sched_Stage_1_NMS(float var_nodes[_N],
//		float var_mesgs[_M], unsigned int PosNoeudsVariable[_M],
//   		unsigned int loops
//		);


CGPU_Decoder_NMS::CGPU_Decoder_NMS(int _nb_frames, int block_size, unsigned int n, unsigned int k, unsigned int m):
		CGPUDecoder(_nb_frames, block_size, n, k, m)
{
	...
	cudaFuncGetAttributes(&attr, LDPC_Sched_Stage_1_NMS);
...
CGPU_Decoder_NMS::~CGPU_Decoder_NMS()
{

}

void CGPU_Decoder_NMS::initialize()
{

}

void CGPU_Decoder_NMS::decode(float Intrinsic_fix[_N], int Rprime_fix[_N], int nombre_iterations)
{
    cudaError_t Status;
	...
	LDPC_Sched_Stage_1_NMS<<<nb_blocks, BLOCK_SIZE>>>(device_V, d_MSG_C_2_V, d_transpose, nombre_iterations);
