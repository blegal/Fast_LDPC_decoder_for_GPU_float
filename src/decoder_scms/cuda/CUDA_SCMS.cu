...
//#define OFFSET		0.00
#define OFFSET		0.15
#define signc(_X)	((_X<0.0)?(unsigned int)0:(unsigned int)1)		// 1 => NEGATIF
#define fabsc(_X)	((_X<0.0)?-_X:_X)	//
#define invc(S,_X)	((S==1)?_X:-_X)		// SI 1=> ALORS CHANGEMENT DE SIGNE

#define MERGE_WITH_LAST_MSG_SIGN(a,b)   (__int_as_float( ((__float_as_int(a)) & 0xFFFFFFFC) | b))
#define GET_MSG_VALUE(a)                (__int_as_float( ((__float_as_int(a)) & 0xFFFFFFFC)))
#define GET_LAST_MSG_SIGN(a)            (                 __float_as_int(a)   & 0x00000001)
#define GET_LAST_MSG_ERASE(a)           (                 __float_as_int(a)   & 0x00000002)

__global__ void __launch_bounds__(128, 2) LDPC_Sched_Stage_1_SCMS(float var_nodes[_N],
	...
	loops -= 1;
	{
		...
		for (int z = 0; z <DEG_1_COMPUTATIONS; z++) {
			...
			for (int j = 0; j < DEG_1; j++) {
				...
				*p_msg1w           = MERGE_WITH_LAST_MSG_SIGN(msg_sortant, signc(tab_vContr[j])); // MODIFIED SCMS
				p_msg1w           += ii;
				int iAddr          = iTable[j] * ii + i;
	...
	while( loops-- ){
		...
		for (int z = 0; z <DEG_1_COMPUTATIONS; z++) {
			...
			#pragma unroll
			for (int j = 0; j<DEG_1; j++) {
				float CN_to_VN       = GET_MSG_VALUE( prefetch_msg[j] );
				float theContrib     = tab_vContr[j] - CN_to_VN;
				tab_vFinal[j]        = theContrib;
                bool last_msg_erased = GET_LAST_MSG_ERASE(prefetch_msg[j]) != 0;
                bool m_switch        = (GET_LAST_MSG_SIGN(prefetch_msg[j]) != signc(tab_vContr[j]));

                if( last_msg_erased == true ){
                    tab_vContr[j]   = theContrib;
                    tab_vMask[j]    = signc(tab_vContr[j]);
			...
			for (int j = 0; j < DEG_1; j++) {
				int iAddr          = iTable[j] * ii + i;
				var_nodes[ iAddr ] = tab_vFinal[j] + output_msg[j];
				*p_msg1w           = MERGE_WITH_LAST_MSG_SIGN(output_msg[j], tab_vMask[j]); // MODIFIED SCMS
				p_msg1w           += ii;
			}
		}
#if NB_DEGRES > 1
