...
#include "CGPU_Decoder_MS.h"
#include "../transpose/GPU_Transpose.h"

CGPU_Decoder_MS::CGPU_Decoder_MS(int _nb_frames, int block_size, unsigned int n, unsigned int k, unsigned int m):
CGPUDecoder(_nb_frames, block_size, n, k, m)
{
	...
	cudaFuncGetAttributes(&attr, LDPC_Sched_Stage_1_MS);
...
CGPU_Decoder_MS::~CGPU_Decoder_MS()
{

}

void CGPU_Decoder_MS::initialize()
{

}

void CGPU_Decoder_MS::decode(float Intrinsic_fix[_N], int Rprime_fix[_N], int nombre_iterations)
{
    cudaError_t Status;
...
//	printf("processing decode (%d, %d, %d) !\n", nb_blocks, BLOCK_SIZE, nombre_iterations);
	LDPC_Sched_Stage_1_MS<<<nb_blocks, BLOCK_SIZE>>>(device_V, d_MSG_C_2_V, d_transpose, nombre_iterations);
