...
#include "CGPU_Decoder_OMS.h"
#include "../transpose/GPU_Transpose.h"

//extern __global__ void LDPC_Sched_Stage_1_OMS(float var_nodes[_N],
//		float var_mesgs[_M], unsigned int PosNoeudsVariable[_M],
//   		unsigned int loops
//		);


CGPU_Decoder_OMS::CGPU_Decoder_OMS(int _nb_frames, int block_size, unsigned int n, unsigned int k, unsigned int m):
		CGPUDecoder(_nb_frames, block_size, n, k, m)
{
	...
	cudaFuncGetAttributes(&attr, LDPC_Sched_Stage_1_OMS);
  	...
  	printf("(II) LDPC_Sched_Stage_1_MS (PTX version %d)\n", attr.ptxVersion);
...
CGPU_Decoder_OMS::~CGPU_Decoder_OMS()
{

}

void CGPU_Decoder_OMS::initialize()
{

}

void CGPU_Decoder_OMS::decode(float Intrinsic_fix[_N], int Rprime_fix[_N], int nombre_iterations)
{
    cudaError_t Status;
	...
	LDPC_Sched_Stage_1_OMS<<<nb_blocks, BLOCK_SIZE>>>(device_V, d_MSG_C_2_V, d_transpose, nombre_iterations);
